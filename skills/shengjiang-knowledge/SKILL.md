...
1. 先确认账号方向、目标人群、内容平台和主要产出形式；未知项标“待确认”，不编造定位。
2. 新库使用 `--profile creator` 预览；已有库不重建，先给目录映射和写入预览。
3. 对标原文、评论和逐字稿保留平台、作者、链接、抓取日期和原始路径。
4. 选题、草稿、定稿和已发布内容按状态流转，不复制多个“最终版”。
5. 发布后把平台、发布时间、标题、链接和真实数据写回发布复盘；没有数据就留空。
6. 批量导入对标资料、调整内容流程或积累一轮发布数据后，重新运行健康检查。
...
10. `system/MEMORY_LOG.md` 中是否有同类纠正已达 3 次却还没升级为固定规则。
...
1. 用户纠正输出或判断，或明确说“记住这个教训”“别再犯”时，立刻按 `system/MEMORY_LOG.md` 的格式追加一条。
2. 写清场景、用户纠正、后续行动，并标注“该类纠正第 N 次”。判断同类看错误模式，不看字面是否相同。
3. 同类纠正达到 3 次，主动提议升级到 `system/PROCEDURES.md`。升级必须让用户确认，不能自动改长期规则。
4. 重要任务开始前，先读取 `MEMORY_LOG.md` 最近的纠正；升级后的 `PROCEDURES.md` 随入口加载。
5. 不记录密钥、隐私和敏感内容，只记录错误模式与正确做法；记录默认只增不改。
